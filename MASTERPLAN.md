...
- `godot/assets/` directory created with 8 pack subfolders
- Asset inventory README at `godot/assets/README.md`
- FBX/GLB files committed to repo (43MB total)
- (Per-file .import defaults will be auto-generated by Godot on first editor open)

#### D.2 — Vehicle model integration (Quaternius Cars) ✅
- VehicleData.gd: added CAR_MODELS dict mapping 7 model names to FBX paths
- VehicleData.gd: VEHICLE_POSITIONS now references model names (not colors)
- Vehicle.gd _build_mesh(): tries to load real FBX model first
  - Falls back to box mesh if asset missing (graceful degradation)
...
- Vehicle.gd _animate_wheels_and_body(): new function called every physics frame
  ...
  - Body pitch: squat on accel, dive on brake (target_pitch = -throttle * 0.03)
    Smoothly lerped, max ~1.7° for subtle weight transfer feel
- GameScene.gd _spawn_vehicles(): passes model name (not color) to Vehicle
