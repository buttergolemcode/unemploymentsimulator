...
static func _make_road(parent: Node3D, axis: String, pos: float, half_w: float, sw: float, is_main: bool):
	var length = 320.0
	# Asphalt
	var asphalt = CSGBox3D.new()
	if axis == "x":
		asphalt.size = Vector3(length, 0.02, half_w * 2)
		asphalt.position = Vector3(0, 0.02, pos)
	else:
		asphalt.size = Vector3(half_w * 2, 0.02, length)
		asphalt.position = Vector3(pos, 0.02, 0)
	var amat = StandardMaterial3D.new()
	amat.albedo_color = Color(0.09, 0.09, 0.09)
	amat.roughness = 0.95
	asphalt.material = amat
	...
	for i in count:
		...
		if axis == "x":
			dash.size = Vector3(1.5, 0.01, 0.15)
			dash.position = Vector3(t, 0.04, pos)
		else:
			dash.size = Vector3(0.15, 0.01, 1.5)
...
static func _build_map_borders(parent: Node3D) -> void:
	...
	for i in range(-260, 261, 30):
		var x = i + (randf() - 0.5) * 20
		var z = -220 - randf() * 40
		var h = 35 + randf() * 20
		var r = 25 + randf() * 15
		var m = CSGCylinder3D.new()
		m.radius = r
		m.height = h
		m.sides = 6
		m.position = Vector3(x, terrain_height(x, z) + h / 2, z)
	...
	for i in range(20):
		var z = -100 + i * 12
		var b = CSGBox3D.new()
		b.size = Vector3(1.2, 1.2, 8)
		b.position = Vector3(240, by + 0.6, z)
		var mat = StandardMaterial3D.new()
	...
	sign.size = Vector3(4, 1, 0.1)
	sign.position = Vector3(240, by + 3, 0)
	var smat = StandardMaterial3D.new()
	...
	for i in range(40):
		...
		post.radius = 0.05
		post.height = 3
		post.position = Vector3(-240, fy + 1.5, z)
		var mat = StandardMaterial3D.new()
		mat.albedo_color = Color(0.29, 0.29, 0.23)
		post.material = mat
		parent.add_child(post)
	# Warning sign
	var wsign = CSGBox3D.new()
	wsign.size = Vector3(2, 1.5, 0.1)
	...
	wsign.material = wsmat
	parent.add_child(wsign)
	# Watchtower
	var tower = CSGBox3D.new()
	tower.size = Vector3(3, 8, 3)
