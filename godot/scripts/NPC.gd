...
static var STREET_POSITIONS: Array = [-200, -100, 0, 100, 200, 300]
...
static func _is_on_street(x: float, z: float) -> bool:
	...
	for pos in STREET_POSITIONS:
		...
		if abs(z - pos) < ROAD_HALF and abs(x) > -250 and abs(x) < 350:
			return true
		# North-South street at x=pos
		if abs(x - pos) < ROAD_HALF and abs(z) > -350 and abs(z) < 450:
			return true
	return false
